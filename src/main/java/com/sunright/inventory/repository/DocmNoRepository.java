...
public interface DocmNoRepository extends PagingAndSortingRepository<DocmNo, DocmNoId>, JpaSpecificationExecutor<DocmNo> {
    ...
    @Query(value = "SELECT prefix || lpad(to_char(last_generated_no + 1), 5, '0') as generatedNo, nvl(last_generated_no, 0) + 1 as docmNo " +
            "FROM DOCM_NO docm " +
            "where company_code = :companyCode and plant_no = :plantNo and type = :type and sub_type = :subType", nativeQuery = true)
